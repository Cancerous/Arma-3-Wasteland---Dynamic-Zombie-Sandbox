...
private ["_wait","_pendingunits","_units","_deadunits","_oa","_manwait","_vehiclewait","_removedead"];
	...
	sleep 0.01;
	{
		if (isNil {_x getVariable "CLY_removedead"}) then {
			if !(_x in CLY_removedeadpending) then {
				...
				if (_wait>0 and vehicle _x==_x and count magazines _x==0 and count weapons _x==0 and count items _x==0) then {
